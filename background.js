...
//デッキのタブのズームを無効化する。sender 検証 → タブの現在の url の再確認 → 無効化中で倍率が既定からずれていれば automatic へ戻す → setZoomSettings の順に進み、
...
//ズームを無効化しているのに倍率が既定からずれているタブを、無効化し直す前にいったん automatic へ戻す。
...
//mode が disabled でも倍率が既定 (getZoomSettings の defaultZoomFactor) のままなら固定は生きているので、何もせず done(true) を呼ぶ。
//  プロファイル切替は同じ document 上でデッキを組み直して無効化を依頼し直すため、ここで切り替えると x.com のブラウザ標準ズームで一瞬描画されてから戻る。
//API の失敗は warn して done(false) を呼ぶ。automatic へ戻すのは倍率が既にずれているときだけなので、その後の失敗で依頼前より悪い状態にはならない
function release_disabled_zoom_mode(tab_id, done){
    chrome.tabs.getZoomSettings(tab_id, function(zoom_settings){
        ...
        chrome.tabs.getZoom(tab_id, function(zoom_factor){
            if(chrome.runtime.lastError){
                console.warn("deck_zoom_disable failed->", chrome.runtime.lastError.message);
                done(false);
                return;
            }
            if(is_same_zoom_factor(zoom_factor, zoom_settings.defaultZoomFactor)){
                done(true);
                return;
            }
...
//ズーム倍率の一致判定。倍率は内部のズームレベルから浮動小数で算出されるため、丸め誤差を許容して比べる。
//どちらかが数値でなければ一致とみなさない (defaultZoomFactor が無いブラウザでは固定のやり直し側に倒す)
const ZOOM_FACTOR_TOLERANCE = 0.001;
function is_same_zoom_factor(a, b){
    return typeof a === "number" && typeof b === "number" && Math.abs(a - b) < ZOOM_FACTOR_TOLERANCE;
}
