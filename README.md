...
* 表示サイズ設定 (文字サイズ / UI サイズ / カラム内容のサイズを % で別々に設定。デッキのタブではブラウザ標準ズーム (Ctrl +/-) を無効化し、この設定で拡大縮小する。ズーム無効化は Chromium 系ブラウザのみで、Firefox では効かない。100% はブラウザ既定倍率での表示。カラム内容のサイズは Firefox では未検証)
