...
* 見えていないタブでの自動更新の停止 (Open-Deck のタブが別タブの裏・最小化などで隠れているあいだは全カラムの自動更新を止め、表示に戻ったとき最後の更新から設定した間隔以上経ったカラムだけを 1 回すぐ更新する。判定はブラウザの可視状態 (Page Visibility API) に従うため、非アクティブでも画面に見えているウィンドウや一部が重なっているだけのウィンドウでは更新を続け、別のウィンドウに完全に覆われたウィンドウで止まるのは Windows 版 Chrome のみ)
